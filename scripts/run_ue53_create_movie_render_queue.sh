...
export BEDLAM_MRQ_STATUS_DIR="${BEDLAM_MRQ_STATUS_DIR:-$BEDLAM_MRQ_OUTPUT_DIR}"
...
STATUS_PATH="$BEDLAM_GENERATED_ASSET_STORE/linux_mrq_generation_status.json"
...
if ! grep -q '"status": "complete"' "$STATUS_PATH" 2>/dev/null; then
    echo "ERROR: MRQ generation did not complete successfully: $STATUS_PATH" >&2
    test ! -f "$STATUS_PATH" || cat "$STATUS_PATH" >&2
    exit 1
fi
echo "MRQ generation completed. Status: $STATUS_PATH"
